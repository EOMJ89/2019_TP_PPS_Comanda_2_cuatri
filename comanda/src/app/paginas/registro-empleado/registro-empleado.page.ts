...
export class RegistroEmpleadoPage implements OnInit
{
  ...
  private listaPerfil:Array<string> = ["dueño","supervisor","mozo","bartender","candybar","camarero"];
  ...
  */
  ...
  public EscanearDNI()
  {
    const options: BarcodeScannerOptions = { prompt: 'Escaneé el DNI', formats: 'PDF_417' };
    this.barcodeScanner.scan(options).then((barcodeData: BarcodeScanResult) => {
      const scan = (barcodeData.text).split('@');
      this.usuario.DNI = parseInt(scan[1]);
      this.usuario.apellido = scan[4];
      this.usuario.nombre = scan[5];
    }, (err) => {
      alert(err);
    });
  }
